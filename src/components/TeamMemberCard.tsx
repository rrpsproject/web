...
export default function TeamMemberCard({ member }: { member: TeamMember }) {
  ...
  return (
    <div className="flex w-40 flex-col items-center text-center">
      <div className="flex h-20 w-20 items-center justify-center overflow-hidden rounded-full bg-brand-cream-deep text-lg font-semibold text-brand-navy/60">
        {member.photo ? (
      ...
      <p className="mt-1 text-xs leading-snug text-brand-navy/65">{member.bio}</p>
    </div>
  );
}
